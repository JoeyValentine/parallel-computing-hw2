 ...
 *           This version uses conditional variables and mutex
 * 
 * Compile:  gcc -g -Wall -o pth_cv_mutex pth_cv_mutex.c 
 *              my_rand.c -lpthread
 *           needs timer.h and my_rand.h
 * Usage:    ./pth_cv_mutex <thread_count>
...
pthread_cond_t      safe_to_read;
pthread_cond_t      safe_to_write;
pthread_mutex_t     read_mutex;
pthread_mutex_t     write_mutex;
pthread_mutex_t     op_count_mutex;
...
#  endif
   ...
   pthread_cond_init(&safe_to_read, NULL);
   pthread_cond_init(&safe_to_write, NULL);
   pthread_mutex_init(&read_mutex, NULL);
   pthread_mutex_init(&write_mutex, NULL);
   pthread_mutex_init(&op_count_mutex, NULL);
...
#  endif
   ...
   Free_list();
   pthread_mutex_destroy(&op_count_mutex);
   pthread_mutex_destroy(&read_mutex);
   pthread_mutex_destroy(&write_mutex);
   pthread_cond_destroy(&safe_to_read);
   pthread_cond_destroy(&safe_to_write);
   free(thread_handles);
...
void* Thread_work(void* rank) {
   ...
   for (i = 0; i < ops_per_thread; ++i) 
   {
      ...
      if (which_op < search_percent) 
      {
         ...
         while (write_count == 1)
           pthread_cond_wait(&safe_to_read, &read_mutex);
         ++read_count;
         ...
         --read_count;
         if (read_count == 0)
           pthread_cond_signal(&safe_to_write);
         pthread_mutex_unlock(&read_mutex);
      ...
      else if (which_op < search_percent + insert_percent) 
      {
         ...
         while (read_count || write_count)
           pthread_cond_wait(&safe_to_write, &write_mutex);
         write_count = 1;

         Insert(val);
         
         write_count = 0;
         pthread_cond_signal(&safe_to_read);
